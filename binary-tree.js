...
class BinaryTree {
  ...
  nextLarger(lowerBound) {
    ...
    //using the Breadth First Search (BFS) (queue)
    let queue = [this.root];
    let smallest = null;
    ...
    while (queue.length) {
      ...
      let shouldReassignSmallest = currentVal < smallest || smallest === null;

      if (largerThanLowerBound && shouldReassignSmallest) {
        smallest = currentVal;
      }
  ...
  static deserialize(stringTree) {
    ...
    const buildTree = () => {
      ...
      if (values.length) {
        ...
        //convert the values back into nums.
